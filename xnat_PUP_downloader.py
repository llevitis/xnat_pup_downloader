...
def get_image_and_header(assessor_puptimecourse_obj,
    ...
    try: 
        ...
        assessor_puptimecourse_obj.resource('DATA').file(orig_img_path).get(new_img_path)
        if orig_hdr_path is not None and new_hdr_path is not None:
            assessor_puptimecourse_obj.resource('DATA').file(orig_hdr_path).get(new_hdr_path)
...
def get_orig_pet_info_file(puptimecourse_obj,
    ...
    new_img_path =  os.path.join(ses_dir, "sub-" + sub + "_ses-" + ses + "_task-rest_acq-pib_pet.info")
    if not os.path.exists(new_img_path):
        get_image_and_header(puptimecourse_obj, orig_img_path, None, new_img_path, None)
    else: 
...
def main():
    parser = argparse.ArgumentParser(description='Script to download PET Unified Pipeline derivatives from XNAT server.')
